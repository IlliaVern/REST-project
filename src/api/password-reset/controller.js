...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your REST-project account.<br>
        ...
        &mdash; REST-project Team
      `
      return sendMail({ toEmail: email, subject: 'REST-project - Password Reset', content })
    })
